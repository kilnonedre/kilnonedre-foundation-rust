...
pub async fn grpc_load_relation_map<OwnerId, Rel, Target, FutRel, FutTarget>(
...
) -> Result<HashMap<OwnerId, Vec<Target>>, Status>
where
    OwnerId: Eq + Hash + Copy,
    Target: Clone,
    FutRel: Future<Output = Result<Vec<Rel>, Status>>,
    FutTarget: Future<Output = Result<Vec<Target>, Status>>,
{
...
pub async fn grpc_load_relation_map_with<
    OwnerId,
    Rel,
    Target,
    Value,
...
>(
    ...
    target_id: FTargetId,
    value_fn: FValue,
) -> Result<HashMap<OwnerId, Vec<Value>>, Status>
where
    OwnerId: Eq + Hash + Copy,
    Value: Clone,
    FutRel: Future<Output = Result<Vec<Rel>, Status>>,
